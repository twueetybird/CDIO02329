...
Start_Betting_WithWheelHistory:
	...
	LD R0, NEWLINE
    	...
    	; Load new spin into R3 (will be inserted at index 0)
    	LD R0, WINNING_VALUE_ADDR
    	LDR R3, R0, 0         ; R3 = new spin (carry value to insert)

    	;Shift values down using bucket brigade logic
    
    	LD R1, WheelHistory_Start   ; R1 = address of index 0

    	AND R4, R4, 0              ; R4 = loop counter
    	ADD R4, R4, 10             ; We will repeat 10 times

ShiftLoop:
    	LDR R2, R1, 0              ; R2 = value currently at R1 (new carry)
    	STR R3, R1, 0              ; store previous value
    	ADD R3, R2, 0              ; copy current value into R3 for next round
    	ADD R1, R1, 1              ; move to next index
    	ADD R4, R4, -1
    	BRp ShiftLoop

    	; Print values from index 0 -> 9 (10 values)
    	LD R1, WheelHistory_Start
    	AND R4, R4, 0
    	ADD R4, R4, 10

PrintLoop:
    	LDR R6, R1, 0             ; R6 = spin value
    	LD R2, ASCII_0
    	ADD R0, R6, R2
    	OUT
...
; Converts number in R0 to string at ASCII_BUFFER 
; ( This number to string converter has been made with help from ChatGTP)
Num_to_string:
...
FAST:				; Here is where we set the fast speed value (Fast is more of a hard spin so long spin)
	AND R0, R0, 0
	ADD R0, R0, 9		; R0 is holding the range we want our random number to be in between
...
SLOW_DOWN1	.FILL -1013
SLOW_DOWN2	.FILL -1443
SLOW_DOWN3	.FILL -2329
SLOW_DOWN4	.FILL -4119
...
LostMSG			.stringz "Wrong and lost your bet"
